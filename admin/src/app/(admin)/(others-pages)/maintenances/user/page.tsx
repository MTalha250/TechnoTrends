...
import { Search, Calendar, Wrench, Filter } from "lucide-react";
import useAuthStore from "@/store/authStore";
import { getUserMaintenances } from "@/hooks/maintenances";
...
export default function UserMaintenancesPage() {
  ...
  const fetchMaintenances = async () => {
    ...
    try {
      setLoading(true);
      const data = await getUserMaintenances(token);
    ...
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-emerald-500 to-emerald-700 rounded-2xl p-6">
        ...
        <div className="relative flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white">My Maintenances</h1>
            <p className="text-emerald-100 text-sm mt-0.5">
              {filteredMaintenances.length} assigned {filteredMaintenances.length === 1 ? "maintenance" : "maintenances"}
            </p>
          </div>
        </div>
      </div>
                  ...
                  <td colSpan={5} className="py-12">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <p className="text-gray-500 dark:text-gray-400 font-medium">
                        {search || statusFilter !== "All" ? "No maintenances match your filters" : "No assigned maintenances"}
                      </p>
                      <p className="text-gray-400 dark:text-gray-500 text-sm mt-1">
                        {search || statusFilter !== "All" ? "Try adjusting your search or filters" : "You have not been assigned to any maintenances yet"}
                      </p>
                    </div>
                  </td>
                </tr>
